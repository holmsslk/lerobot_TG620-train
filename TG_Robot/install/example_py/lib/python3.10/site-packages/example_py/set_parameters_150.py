...
def get_user_input():
    ...
    if mode_choice == "1":
        ...
        if 5 in selected_params:
            ...
            print("最大速度参数")
            print("-" * 80)
            max_vel_str = input("请输入最大速度 (°/s², 默认150.0): ").strip()
...
def main():
    ...
    # 运动参数配置（批量模式下所有设备都设置运动参数）
    devices_need_motion_params = [1, 2, 3, 4, 5, 6]
    # 设备1、2、3的电流为10A，设备4、5、6的电流为3A
    default_max_current_123 = 10.0
    default_max_current_456 = 3.0
    ...
    if batch_mode:
        ...
        for config in device_configs:
            ...
            if config['id'] in devices_need_motion_params:
                current_value = default_max_current_123 if config['id'] in [1, 2, 3] else default_max_current_456
                limit_info += f" + 电流:{current_value}A, 加速度:{default_max_acceleration}°/s², 减速度:{default_max_deceleration}°/s²,速度:{default_max_velocity}°/s²"
            print(limit_info)
    else:
        # 单设备模式
        ...
        if 2 in selected_params:
            print(f"    最大电流: {config.get('max_current', default_max_current_123)} A")
        if 3 in selected_params:
        ...
        if 5 in selected_params:
            print(f"    最大速度: {config.get('max_velocity', default_max_velocity)}°/s²")
    print("=" * 80)
    ...
    for config in device_configs:
        if batch_mode:
            ...
            if config['id'] in devices_need_motion_params:
                operations_per_device += 4  # 电流、加速度、减速度、速度
        else:
            # 单设备模式：根据选择的参数计算
            selected_params = config.get('selected_params', {1, 2, 3, 4, 5})
    ...
    for idx, config in enumerate(device_configs, 1):
        ...
        pos_max = 0.0
        enable_limit = 0
        max_current = default_max_current_123 if device_id in [1, 2, 3] else default_max_current_456
        ...
        if batch_mode:
            ...
            max_current = config.get('max_current', default_max_current_123 if device_id in [1, 2, 3] else default_max_current_456)
        ...
        if (batch_mode and include_motion_params) or (not batch_mode and include_motion_params):
            ...
            if 5 in selected_params:
                print("\n" + "-" * 80)
                print(f"[{step_num}/{operations_per_device}] 设置最大速度")
                print("-" * 80)
                sent, verified = setter.write_parameter_float32(device_id, PARAM_MAX_VELOCITY_MAX, max_velocity, use_canfd=True)
                if sent:
                    device_success += 1
                    total_success_count += 1
                    if verified:
