...
class CANFrictionSetter:
    def __init__(self, interface='can0'):
        ...
        self.PARAM_ADDRESS_MAP = {
            'voltage_per_amp': 17,               # 电流采样系数（单位V/A）
            'voltage_k': 18,                     # 电压采样系数
        ...
        self.MODULE_CONFIGS = {
            '3NM': {
                'voltage_per_amp': 0.255,
                'voltage_k': 0.02206148,
                'torque_per_amp': 3.51204,
                'friction_enable': 1,
    ...
    def set_joint_friction_parameters(self, device_id, module_type):
        ...
        operations = [
            ('voltage_per_amp', config['voltage_per_amp'], 'float32', '电流采样系数'),
            ('voltage_k', config['voltage_k'], 'float32', '电压采样系数'),
...
def main():
    ...
    total_operations = len(joint_configs) * 10  # 每个关节10个参数
